...
#+LATEX_HEADER: \usepackage{scicite}
...
#+BEGIN_EXPORT latex
% The author list
\title{An elastic interfacial transistor enabled by
  superhydrophobicity}

\author {Tian Tian,$^{1}$ Chander Shekhar Sharma,$^{2,\dagger}$
  Navanshu Ahuja,$^{1}$ \\ Matija Varga,$^{3}$ Raja
  Selvakumar,$^{1,\ddagger}$ Yen-Ting Lee,$^{4,5}$ \\Yu-Cheng
  Chiu,$^{4}$ Chih-Jen Shih,$^{1,*}$\\
  \\
  \normalsize{$^{1}$Institute for Chemical and Bioengineering, ETH
    Z{\"{u}}rich,}\\
  \normalsize{CH-8093 Z{\"{u}}rich, Switzerland}\\
  \normalsize{$^{2}$Laboratory of Thermodynamics in Emerging
    Technologies, ETH Z{\"{u}}rich,}\\
  \normalsize{ CH-8092
    Z{\"{u}}rich, Switzerland}\\
  \normalsize{$^{3}$Electronics Laboratory, ETH Z{\"{u}}rich, CH-8092 Z{\"{u}}rich, Switzerland.}\\
  \normalsize{$^{4}$Department of Chemical Engineering, }\\
  \normalsize{National Taiwan University of Science and Technology, Taipei 10607, Taiwan.}\\
  \normalsize{$^{5}$National Synchrotron Radiation Research Center, Hsinchu 30076, Taiwan.}\\
  \normalsize{$^{\dagger}$Present address: Department of Mechanical Engineering,}\\
  \normalsize{Indian Institute of Technology Ropar, Rupnagar, Punjab 140001, India.}\\
  \normalsize{$^{\ddagger}$Present address: Department of Chemical and Biomolecular Engineering, }\\
  \normalsize{University of California, Berkeley, CA 94720, USA}\\
  \\
  \normalsize{$^{*}$To whom correspondence should be addressed; E-mail: chih-jen.shih@chem.ethz.ch}\\
}
#+END_EXPORT

#+LaTeX: \baselineskip24pt
#+LaTeX: \maketitle
